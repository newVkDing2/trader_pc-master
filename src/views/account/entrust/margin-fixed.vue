<template >
  <div class="fit-background margin-fixed">
    ...
    <div class="table-bg fit-background">
      ...
      <el-table :data="tableData" style="width: 100%" v-loading="requestStatus">
        <div v-for="(item, index) in ordersTableColumn" :key="item.prop + '_' + index">
          ...
          <el-table-column v-else-if="item.prop == 'fxMeun'" v-bind="item">
            <template slot-scope="scope">
              <span v-if="directionTypeList[scope.row.direction]" :class="[
          ...
          <el-table-column v-else-if="item.prop == 'ztczMeun'" v-bind="item" label-class-name="statusColor">
            <template slot="header">
              <span class="top" v-if="navCurrent == 0 || navCurrent == 3">
                {{
                                currCoinLangFun("operation", "all")
                                }}
              </span>
              <span class="top" v-if="navCurrent == 1 || navCurrent == 4">
                {{
                                currCoinLangFun("orders_table_state", "full")
                                }}
              </span>
              <span class="top" v-if="navCurrent == 2">
                {{
            ...
            <template slot-scope="scope">
              ...
              <span v-if="navCurrent == 4">{{ scope.row.borrow_status }}</span>
      ...
      <!--  还款-->
      <repayment-coin ref="repaymentCoin" tradeType="zc" :orderData="currOrderItem"></repayment-coin>
    </div>
  </div>
</template>
            ...
            if (this.navCurrent == 0 || this.navCurrent == 1) {
              ...
              params = {
                ...
                cjjl_sl:
                  item.direction == 1
                    ? toThousands(Number(item.balance).myAdd(item.fee_balance))
                    : toThousands(Number(item.frozen_balance)),
